...
clr_isi1_serv_home <- complr(
  data = dpsg[isig == 1 & servicetype == 1],
  parts = parts,
  # idvar = "record_id",
  total = 1440
)
...
m_tib_isi1_serv_home <- brmcoda(clr_isi1_serv_home,
  mvbind(z1_1, z2_1, z3_1, z4_1, z5_1) ~
    s(bedrest_min) +
    age + female + bmi + white + working + s(perHrAHSleep),
...
clr_isi1_serv_hospital <- complr(
  data = dpsg[isig == 1 & servicetype == 0],
  parts = parts,
  # idvar = "record_id",
  total = 1440
)

m_tib_isi1_serv_hospital <- brmcoda(clr_isi1_serv_hospital,
  mvbind(z1_1, z2_1, z3_1, z4_1, z5_1) ~
    s(bedrest_min) +
    age + female + bmi + white + working + s(perHrAHSleep),
  iter = 4000, chains = 6, cores = 6, seed = 123, warmup = 1000,
...
summary(m_tib_isi1_serv_hospital)
saveRDS(m_tib_isi1_serv_hospital, file.path(out, "m_tib_isi1_serv_hospital.rds"))

m_tib_isi1_serv_home <- readRDS(file.path(out, "m_tib_isi1_serv_home.rds"))
m_tib_isi1_serv_hospital   <- readRDS(file.path(out, "m_tib_isi1_serv_hospital.rds"))

### predictions -----------------------
for (serv in c("serv_home", "serv_hospital")) {
 # Select model and output names
 model <- if (serv == "serv_home") m_tib_isi1_serv_home else m_tib_isi1_serv_hospital
...
pred_tibq5_isi1_serv_hospital_sum <- readRDS(file.path(out, "pred_tibq5_isi1_serv_hospital_sum.rds"))

pred_tibq5_isi1_serv_home_sum[, serv := "home"]
pred_tibq5_isi1_serv_hospital_sum[, serv := "hospital"]
colnames(pred_tibq5_isi1_serv_home_sum)
colnames(pred_tibq5_isi1_serv_hospital_sum)

pred_tibq5_isi1_serv_sum <- rbind(
  pred_tibq5_isi1_serv_home_sum,
  pred_tibq5_isi1_serv_hospital_sum
)
...
plot_min_params <- list(
  ...
  "SOL" = list(
    limits = c(-150, 650), breaks = c(100, 200, 300), breaks2 = c(.01, .04, .07), name = "SOL", y_offset = 1.5
  ),
  "WASO" = list(
    limits = c(-20, 200), breaks = c(50, 100, 150), breaks2 = c(.03, .05, .07, .09), name = "WASO", y_offset = 1.5
  ),
  "N1+2" = list(
...
make_min_plot <- function(part_label) {
  ...
  ggplot(
    ...
    scale_shape_manual(values = shape_min_serv) +
...
grDevices::cairo_pdf(
  file = file.path(out, "plot_serv_min.pdf"),
  width = 10,
  height = 14,
)
ggarrange(
...
plot_perc_params <- list(
  ...
  # ),
  "N1+2" = list(
    limits = c(40, 100), breaks = c(60, 70, 80), name = "N1+2", y_offset = 0.01
  ),
  "N3" = list(
...
grDevices::cairo_pdf(
  file = file.path(out, "plot_serv_perc.pdf"),
  width = 5,
  height = 14,
)
ggarrange(
...
# combined plot ------------------------
grDevices::cairo_pdf(
  file = file.path(out, "plot_serv_combined.pdf"),
  width = 13,
  height = 13,
)
ggarrange(
...
# table -----------------------------------------------
pred_tibq5_isi1_serv_sum[grepl("q1_q234", par)][, .(serv, par, est_min)]
pred_tibq5_isi1_serv_sum[grepl("q1_q234", par)][, .(serv, par, est_perc)]

pred_tibq5_isi1_serv_sum[grepl("q5_q234", par)][, .(serv, par, est_min)]
pred_tibq5_isi1_serv_sum[grepl("q5_q234", par)][, .(serv, par, est_perc)]

pred_tibq5_isi1_serv_sum[grepl("q1_q5", par)][, .(serv, par, est_min)]
pred_tibq5_isi1_serv_sum[grepl("q1_q5", par)][, .(serv, par, est_perc)]
