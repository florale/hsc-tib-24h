...
m_tib_isi1_female <- brmcoda(clr_isi1_female,
  mvbind(z1_1, z2_1, z3_1, z4_1, z5_1) ~
    s(bedrest_min) +
    s(age) + bmi + white + working + s(perHrAHSleep) + servicetype,
...
for (sex in c("female", "male")) {
 ...
 out_prefix <- paste0("pred_tibq5_isi1_", sex)
 ...
 d_tibq5 <- d_tibq5[!duplicated(d_tibq5[, .(bedrest_min, age, white, perHrAHSleep)]), ]
...
pred_tibq5_sum[grepl("perc", par), est_perc := paste0(format(round(Mean, 2), nsmall = 2), " [", format(round(CI_low, 2), nsmall = 2), ", ", format(round(CI_high, 2), nsmall = 2), "]")]
 ...
 assign(paste0("pred_tibq5_isi1_", sex, "_sum"), pred_tibq5_sum, envir = .GlobalEnv)
 saveRDS(get(paste0("pred_tibq5_isi1_", sex, "_sum"), envir = .GlobalEnv), file.path(out, paste0("pred_tibq5_isi1_", sex, "_sum.rds")))
}
...
pred_tibq5_isi1_female_sum[, female := "Female"]
pred_tibq5_isi1_male_sum[, female := "Male"]
colnames(pred_tibq5_isi1_female_sum)
colnames(pred_tibq5_isi1_male_sum)
...
plot_min_params <- list(
  ...
  "SOL" = list(
    limits = c(-150, 600), breaks = c(100, 200, 300), breaks2 = c(.01, .04, .07), name = "SOL", y_offset = 1.5
  ),
  "WASO" = list(
    limits = c(-20, 250), breaks = c(50, 100, 150), breaks2 = c(.03, .05, .07, .09), name = "WASO", y_offset = 1.5
  ),
  "N1+2" = list(
...
grDevices::cairo_pdf(
  file = file.path(out, "plot_sex_min.pdf"),
  width = 10,
  height = 14,
)
ggarrange(
...
grDevices::cairo_pdf(
  file = file.path(out, "plot_sex_perc.pdf"),
  width = 5,
  height = 14,
)
ggarrange(
...
# combined plot ------------------------
grDevices::cairo_pdf(
  file = file.path(out, "plot_sex_combined.pdf"),
  width = 13,
  height = 13,
)
ggarrange(
...
# table -----------------------------------------------
pred_tibq5_isi1_sex_sum[grepl("q1_q234", par)][, .(female, par, est_min)]
pred_tibq5_isi1_sex_sum[grepl("q1_q234", par)][, .(female, par, est_perc)]

pred_tibq5_isi1_sex_sum[grepl("q5_q234", par)][, .(female, par, est_min)]
pred_tibq5_isi1_sex_sum[grepl("q5_q234", par)][, .(female, par, est_perc)]

pred_tibq5_isi1_sex_sum[grepl("q1_q5", par)][, .(female, par, est_min)]
pred_tibq5_isi1_sex_sum[grepl("q1_q5", par)][, .(female, par, est_perc)]
